...
# Exercise 2
Code chunk is commented out so that it does not download the data every time it is run.
```{r}
...
ggplot(data = discharge_SP_VT, aes(x = dateTime, y = Flow_Inst)) +
  ...
  labs(title = "Water Flow on 10/4/2021 for the Santa Paula and Ventura Rivers",
       x = "Time",
       y = "Flow ft^3/sec")
```
...
Let's try to compare it to the households that speak French. This part is done assuming that the "french_or_french_creole" captures all french speaking households.
```{r}
french <- hh_data %>% 
  filter(Year >= 2009,
         Year <= 2015,
...
```
Something in the French data is causing it to be calculated as NA but I am not sure how this is happening.
